...
import { getEndingArt } from '../design/endingArt';
import { StatBar } from './StatBar';

function EndingArt({ ending }: { ending: EndingResult }) {
  const art = getEndingArt(ending);
  if (!art) return null;
  return <img className="ending-art" src={art.src} alt={art.alt} width={480} height={270} loading="lazy" />;
}
...
export function GameOverScreen({ ending, stats, turn, onRestart, endingStatistic, analyticsConfigured, analyticsPending, analyticsFailed }: { ending: EndingResult; stats: Stats; turn: number; onRestart: () => void } & AnalyticsEndingProps) {
  return <section className="panel ending" role="alert"><p className="stamp">업무 중단</p><h1>{ending.title}</h1><p>{ending.text}</p><EndingArt ending={ending} /><p className="meta">{turn}턴에서 하루가 끝났습니다.</p><EndingRate configured={analyticsConfigured} pending={analyticsPending} failed={analyticsFailed} statistic={endingStatistic} /><StatBar stats={stats} /><button className="primary" onClick={onRestart}>다시 출근하기</button></section>;
}
export function EndingScreen({ ending, stats, onRestart, endingStatistic, analyticsConfigured, analyticsPending, analyticsFailed }: { ending: EndingResult; stats: Stats; onRestart: () => void } & AnalyticsEndingProps) {
  ...
  return <section className="panel ending"><p className="stamp">{ending.kind === 'special' ? '특수 엔딩' : '퇴근 완료'}</p><h1>{ending.grade && `${ending.grade}등급 · `}{ending.title}</h1><p>{ending.text}</p><EndingArt ending={ending} /><strong className="score">{score} <small>/ 400</small></strong>{next.next && <p className="meta">{next.remaining === 0 ? `${next.next}등급은 지표 균형이 더 필요해요` : `${next.next}까지 ${next.remaining}점`}</p>}<EndingRate configured={analyticsConfigured} pending={analyticsPending} failed={analyticsFailed} statistic={endingStatistic} /><StatBar stats={stats} /><div className="actions"><button onClick={copy}>결과 복사</button><button className="primary" onClick={onRestart}>다시하기</button></div></section>;
}
